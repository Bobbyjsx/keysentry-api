...
async def handle_api_key_discovered(
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            await db.commit()
            logger.info(f"Handled API key discovery for user {user_id}")
        except Exception as e:
            await db.rollback()
            logger.error(f"Failed to handle API_KEY_DISCOVERED for user {user_id}: {e}")
